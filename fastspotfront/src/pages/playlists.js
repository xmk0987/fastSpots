...
const handleCreateNewPlaylist = async (event) => {
    ...
    try {
        ...
        const url = `${baseURL}/createPlaylist/${user.id}/${playlistName}/${playlistDescription}/${publicState}`;
...
const addChosen = async (id) => {
    ...
    if(!checkIfSelectedAndOwner(id)){
        ...
        const url = `${baseURL}/addTracks/${id}`;
...
const deleteChecked = async () => {
    if(!checkIfSelectedAndOwner(editorPlaylistId)){
        ...
        const url = `${baseURL}/deleteTracks/${editorPlaylistId}`;
        try{
            const response = await fetch(url, {
                method: 'DELETE',
                headers: {
...
const unfollowPlaylist = async () => {
    if(!editorPlaylistId){
        ...
        try{
            const url = `${baseURL}/unfollowPlaylist/${editorPlaylistId}`;
            const response = await fetch(url, {
                method: 'DELETE',
                headers: {
                    'Content-Type': 'application/json', 
